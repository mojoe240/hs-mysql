...
passport.serializeUser((user, done) => {
  if (user._json.domain !== 'sovrn.com')  done('Invalid Email', null);
  done(null, user.id);
});
    ...
    function(req, username, password, done) { // callback with email and password from our form
        connection.query("SELECT * FROM users WHERE username = ?",[username], function(err, rows){
            if (err)
                return done(err);
            if (!rows.length) {
                return done(null, false, req.flash('errors', { msg: 'No user found'}));
            }
            // if the user is found but the password is wrong
            if (!bcrypt.compareSync(password, rows[0].password))
                return done(null, false, req.flash('errors', { msg: 'Oops! Wrong password.'})); // create the loginMessage and save it to session as flashdata
